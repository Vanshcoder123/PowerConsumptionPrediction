...
def predict():
    try:
        ...
        # Encode categorical values
        building_type_encoded = label_encoders["Building Type"].transform([building_type])[0]
        day_of_week_encoded = label_encoders["Day of Week"].transform([day_of_week])[0]

        # Create input array
        input_data = np.array([[building_type_encoded, square_footage, occupants, appliances, temperature, day_of_week_encoded]])
